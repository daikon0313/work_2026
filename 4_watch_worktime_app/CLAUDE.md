...
### 4. 日次サマリーメール自動送信
毎日朝6時に、前日の作業内容をメールで自動送信する。

| 項目 | 内容 |
|---|---|
| 送信時刻 | 毎日朝6:00 JST |
| 送信先 | 環境変数 `EMAIL_RECIPIENT` で指定 |
| 件名 | 【作業サマリー】YYYY年MM月DD日の作業記録 |
| 本文 | 前日の作業一覧、各作業の時間、合計作業時間 |
...
- アプリ起動中にバックグラウンドで実行
- スケジューラーはアプリ起動時に自動開始
- `.env` ファイルまたは環境変数で送信先を設定

**方法2: GitHub Actions（推奨）**
- GitHub Actions の cron で毎日実行
- ローカルでアプリを起動する必要なし
...
- Calendar API と Gmail API で同じ認証トークンを使用
- メールアドレスは環境変数で管理（GitHubにコミットしない）
...
- Calendar API と Gmail API の両方のスコープを同時に要求する
